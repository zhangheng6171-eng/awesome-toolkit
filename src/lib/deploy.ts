...
const deployRegistry: Record<string, DeployConfig> = {
  ...
  prometheus: {
    ...
    memory_mb: 1024,
    disk_gb: 20,
    post_deploy_url: 'http://你的服务器IP:9090',
    post_deploy_msg: '浏览器访问查看监控指标，默认已监控本机。要监控其他服务器，编辑 prometheus.yml 添加目标地址后重启容器。',
    setup_notes: [
  ...
  passbolt: {
    ...
    post_deploy_msg: '浏览器访问（需通过 HTTPS 自签名证书安全警告），完成安装向导创建管理员账号，务必保存好恢复密钥。安装浏览器插件后即可使用。',
    env_vars: [
      { key: 'DB_PASSWORD', label: '数据库密码', default: 'changeMe123!' },
      { key: 'DB_ROOT_PASSWORD', label: '数据库 Root 密码', default: 'rootChangeMe123!' },
    ],
    setup_notes: [
      '首次访问会提示 SSL 证书不安全，这是正常的（后续可配合 Nginx Proxy Manager 申请正式证书）',
      '创建账号后会生成恢复密钥，务必妥善保存，丢失后将无法恢复账号',
      '团队成员需注册账号后由管理员分配权限',
    ],
  },
  outline: {
    ...
    post_deploy_msg: 'Outline 需要配置 OIDC 登录（推荐 GitHub OAuth 或 Google），部署后需在管理后台配置 OIDC 参数，然后团队成员即可登录使用。',
    env_vars: [
      { key: 'DB_PASSWORD', label: '数据库密码', default: 'changeMe123!' },
      { key: 'SECRET_KEY', label: '应用密钥（至少32字符）', default: 'changeMeToRandom64Chars1234567890abcdefghijklmnopqrstuv' },
      { key: 'UTILS_SECRET', label: '工具密钥（至少32字符）', default: 'changeMeToRandom64Chars0987654321zyxwvutsrqponmlkjih' },
      { key: 'OIDC_CLIENT_ID', label: 'OIDC Client ID（GitHub/Google）', default: '' },
      { key: 'OIDC_CLIENT_SECRET', label: 'OIDC Client Secret', default: '' },
      { key: 'OIDC_AUTH_URI', label: 'OIDC 认证地址', default: '' },
      { key: 'OIDC_TOKEN_URI', label: 'OIDC Token 地址', default: '' },
      { key: 'OIDC_USERINFO_URI', label: 'OIDC 用户信息地址', default: '' },
    ],
    setup_notes: [
  ...
  plausible: {
    ...
    post_deploy_msg: '浏览器访问注册管理员账号，添加你的网站域名，把生成的 JS 统计代码粘贴到网站 <head> 中即可看到实时访客数据。',
    env_vars: [
      { key: 'DB_PASSWORD', label: '数据库密码', default: 'changeMe123!' },
      { key: 'SECRET_KEY_BASE', label: '应用密钥（至少64字符）', default: 'changeMeToRandom64CharsPlausibleSecretKeyBase0000' },
      { key: 'TOTP_VAULT_KEY', label: 'TOTP 加密密钥（至少64字符）', default: 'changeMeToRandom64CharsPlausibleTotpVaultKey000000' },
    ],
    setup_notes: [
      'Plausible 是隐私友好的统计工具，不会追踪个人用户行为',
      '统计代码只有 1KB，不影响网站加载速度',
      '首次部署需要 2-3 分钟初始化 ClickHouse 数据库',
    ],
  },
};
